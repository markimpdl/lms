...
- **Atividade tipo código: C# syntax highlight no CodeMirror 6** (#400): adicionado `@codemirror/lang-csharp@6.1.1` via esm.sh em `src/pages/student/activity/show.php` (~linha 425). 3 linhas, zero setup de bundler — consistente com o padrão dos outros 3 packs (Python, JavaScript, HTML). C# fechou o set das 4 linguagens suportadas pelo Judge0.
- **Atividade tipo código: extensões de upload por linguagem** (#403): `SubmissionStorage::store` recebe `opts['code_language']` e estende a allowlist com a extensão da linguagem (`.py`, `.cs`, `.js`, `.html`). `.zip` continua universal (projetos com vários arquivos), `.txt` como fallback. Tratamento de MIME ambíguo (`text/plain` vira `.py` quando o nome do arquivo é `.py`). Atividades não-código mantêm o default `.pdf/.zip/.txt`. Input `accept` dinâmico + hint i18n parametrizada (`submissions.form.file_hint_dynamic` com `:exts`).

### Correções

- **CRÍTICO: editor CodeMirror + botão Run não apareciam pro aluno em atividade tipo código** (PR #402): bug raiz de E8 (entregue há vários épicos) que ficou silencioso até o PO testar. `ActivitySubmission::findForStudentActivity` (model usado pelo `/student/activity/show.php`) tinha SELECT explícito sem `a.code_language`. Quando E8 adicionou a coluna, esse SELECT específico não foi auditado — `$activity['code_language']` chegava sempre `null`, o `$useEditor` (gate do CodeMirror + botão Run) ficava `false`, aluno via apenas um textarea genérico sem cores e sem botão Executar. Fix: 1 palavra (`a.code_language`) adicionada à query. **Convenção reforçada**: o projeto não usa `SELECT *` — toda coluna nova exige grep + atualizar cada `find*`/`list*`/tree builder manualmente.
- **Bundle: badge "soon" + texto "Judge0 execution arrives in Epic E8" no checkbox `allow_online_code_run`** (PR #402): texto stale em `src/pages/teacher/activity/_form.php` (E8 entregue há vários épicos) que fez o PO desconfiar de marcar a flag. Removido o badge `<span>` e atualizado o hint i18n (PT/EN) pra explicar o que a flag faz quando ON.
- **Ranking: filtro de ano = ano de matrícula, não data do último XP** (PR #404): semântica do filtro `year` mudou completamente. Antes: `YEAR(xp_events.created_at) = :year` (mostrava XP gerado naquele ano). Agora: `EXISTS (SELECT 1 FROM enrollments WHERE student_user_id=u.id AND YEAR(enrolled_at)=:year)` — restringe o ranking aos alunos matriculados no ano. XP exibido permanece cumulativo (não filtra XP por ano). EXISTS evita duplicar a linha do aluno com 2+ matrículas no mesmo ano. Dropdown de anos em `student/ranking.php` e `teacher/ranking.php` agora lê de `enrollments.enrolled_at` (consistente com a nova semântica).
- **DELETE de aluno/curso limpa arquivos físicos do storage** (PR #404): cascade do banco já funcionava (FKs ON DELETE CASCADE em `activity_submissions`, `evaluation_submissions`, `xp_events`, `enrollments`, etc.), mas `Student::delete` e `Course::delete` rodavam apenas o SQL — arquivos em `storage/uploads/tenant_*/` ficavam órfãos. Novo helper `safe_unlink_storage(string $relPath)` em `src/helpers.php` com defesa traversal via `realpath` confinado a `LMS_ROOT/storage/uploads/`. `Student::delete` coleta `stored_path` UNION ALL de `activity_submissions` + `evaluation_submissions` antes do DELETE; pós-commit, `foreach safe_unlink_storage`. `Course::delete` cobre 5 fontes de arquivos (anexos TinyMCE, submissões de atividade, PDFs de enunciado, submissões de avaliação, PDFs Skills Hub gerados) via JOIN com hierarquia `cu→cc→course`.
- **Re-cadastro de LOs com confirmação UI quando há grades** (PR #398): `LearningOutcome::countGradesByCu(int $cuId)` novo (JOIN learning_outcomes × evaluation_submission_lo_grades). Page handler `/teacher/cu/{id}/learning-outcomes` exibe alerta amarelo + checkbox `confirm_drop_grades` quando há notas por LO já gravadas; submit sem confirmação devolve erro `learning_outcomes.err.confirm_required`. Resolve o caso de o professor editar 5 LOs após corrigir submissões e perder as notas por critério via FK CASCADE silenciosa.
- **Logos órfãs ao virar tenant Actvet** (PR #398): novo `Tenant::clearLogo(int $tenantId)` (zera só `logo_path`). `AdminTeachersController::update` detecta transição não-Actvet → Actvet com `logo_path` setada e: (a) zera coluna dentro da tx via `clearLogo`; (b) pós-commit, deleta arquivo via `LogoStorage::deleteByBasename` (best-effort).

### Mudança de comportamento (potencial breaking visual pro PO)

- **Avaliações: sem histórico de tentativas reprovadas** (PR #404): PO: *"Se o aluno foi reprovado e precisou refazer, só é importante a última. Não quero as últimas ocupando espaço no servidor."* `EvaluationSubmissionService::submit` agora **DELETE** a tentativa antiga (cascade limpa `evaluation_submission_lo_grades`) + `safe_unlink_storage` dos arquivos físicos (`stored_path` + `report_pdf_path`) antes de inserir a nova — substitui o soft-delete via `is_current=0`. `attempt` continua incrementando como contador histórico ("aluno está na 2ª tentativa") mesmo sem o registro antigo. Coluna `is_current` deprecated (sempre `1` agora; drop em futura migration). Removidas: seções "Histórico de tentativas" em 3 páginas (`student/evaluation/show.php`, `teacher/evaluation/submission.php`, `teacher/evaluation/submission_quiz.php`), método `EvaluationSubmission::listHistoryForStudent`, campo `history` em `EvaluationSubmission::findForGrading` retorno. XP creditado em `xp_events` não é afetado (UK por `evaluation_id`, não submission). **Migration retroativa**: `public/_cleanup_evaluation_attempts.php` (script web temporário, gate `require_auth` + CSRF, mostrou role logado). Apagou 10 linhas + 9 arquivos órfãos em prod (2026-04-27); script removido do servidor após uso.

### Mudanças internas / Tooling

- **`package.json`** bumpado para 0.29.0.
- **CLAUDE.md**: PHP 8.2 → 8.3 (Hostinger fez auto-upgrade em 2026-04-22; validado por uso real durante todo o roadmap pós-MVP estendido). Features 8.3 liberadas (`typed class constants`, `json_validate`, `#[Override]`); 8.4 ainda proibido.
- **`.claude/skills/code-review.md`**: `PDO via MySQL::pdo()` → `Database::pdo()`; `PHP 8.2+` → `PHP 8.3+`.
- **`doc/99-pendencias-tecnicas.md`**: 4 itens movidos pra "Resolvidas" (E25-02 LO cascade, E24-03 logos órfãs, E0-03 skill Database::pdo, PHP 8.3 status).
- **Higiene GitHub**: 30 issues órfãs fechadas em batch — épicos E23-03, E24, E25, E26, E27, E28, E29 e E30-01 já entregues mas com issues nunca fechadas após merge.
- **`feedback_mpdf_gotchas.md`** (memory): 2 gotchas novas adicionadas — PHP local sem GD silencia imagens (`php -d extension=gd`), `position: absolute` em DIV wrapper funciona em mPDF v8 pra cover layouts.

### Convenções consolidadas nesta janela

- **SELECT explícito é dívida ativa quando se adiciona coluna**: o projeto não usa `SELECT *`. Toda coluna nova em tabela exige `grep -rn "FROM <tabela>"` + atualizar manualmente cada `find*`/`list*`/tree builder. O bug do `code_language` ficou meses no banco antes de manifestar visivelmente — só apareceu quando o PO testou a feature pela primeira vez. Lição: na PR de schema change, fazer essa varredura como ítem de checklist.
- **Cleanup de arquivos físicos pós-DELETE é responsabilidade do model `delete()`, não do banco**: cascade do MySQL não toca o filesystem. Padrão adotado: SELECT paths antes do DELETE, DELETE no banco (cascade limpa relacionados), `foreach safe_unlink_storage` pós-commit (best-effort, falha não rola back). Funciona pra `Student::delete`, `Course::delete`; aplicar o mesmo em qualquer DELETE futuro que toque submissões/anexos.
- **Scripts web temporários ficam em `public/` com prefixo `_`**: `install/` está fora do DocumentRoot por design (proteção pra `schema.sql` e `seed-admin.php` CLI). Pra rodar uma migration via web em prod (PO sem SSH), criar em `public/_cleanup_*.php` com gate de auth + CSRF + instrução clara de "apagar do servidor após uso". URL final acessada por FileZilla pra cleanup. Padrão usado pra `_cleanup_evaluation_attempts.php` nesta release.
- **Texto/badge "soon" / "Epic EX vai entregar" rota pra dívida visual**: quando uma feature chega, é fácil esquecer de remover esses placeholders no UI/i18n. Lição: na release que entrega o épico, varrer `grep -rn "soon\|epic e<N>"` e limpar.
...
- **Bottom tab bar mobile (aluno)** (#369): novo padrão de navegação primária estilo app pra aluno em telas <576px. Partial novo `src/templates/partials/bottom_nav.php` com 4 tabs hardcoded (Início `/student`, Ranking `/student/ranking`, Conquistas `/student/achievements`, Perfil `/profile`) — bell continua no top porque cobre quick-peek + linka pra `/notifications`. Render condicional ao `$isStudentArea` em `layout.php` (mesma fronteira que sidebar e dark mode — 3 features que só existem na área do aluno). Active state via `startsWith` no path (suporta sub-rotas tipo `/profile/theme`). CSS em `student-area.css`: `position: fixed; bottom: 0`, bg `var(--lms-card-bg)` (herda dark auto), shadow leve invertida, `env(safe-area-inset-bottom)` pro iPhone com notch, `z-index: 1030` (acima de conteúdo, abaixo de modal Bootstrap). Body ganha `padding-bottom: 72px` em mobile pra conteúdo não ficar atrás. 1 chave i18n nova `nav.home` (Início/Home); comm audit 1137/1137 sem divergência. Decisão: só pra aluno no MVP — teacher mobile é caso secundário (pode entrar depois se PO sentir falta).
- **/notifications adopta layout student-area + dark mode** (#371): PO clicava em "View all" no sino e caía numa página toda light/sem sidebar — destoava do resto da área do aluno. Agora `/notifications` herda `lms-student-area` quando o user é aluno (sidebar lateral, dark mode automático, bottom nav mobile). Gate em `layout.php`: `$isStudentArea` ganhou path `/notifications` (mantida a check de `role=student` — teacher/super-admin que abrem `/notifications` continuam no layout container Bootstrap clássico). Page handler `src/pages/notifications/index.php` ganhou `$isStudent` local — wrapper `row + col-lg-8` só renderiza pra non-aluno (mesmo padrão de `/profile.php`). 8 overrides dark em `student-area.css` pros componentes `.lms-notif-*` (list bg/border, empty, row border, unread bg violeta translúcido, title/text/time colors, btn hover) — light mode inalterado.

### Correções

- **Navbar mobile esconde Ranking/Conquistas** (#368): PO pediu pra tirar os 2 ícones em telas pequenas (sino + idioma + avatar já eram suficientes; mais nada cabia). 3 regras antigas dentro do `@media 575.98px` que mexiam com `.lms-navbar__link` (padding), `__link-text` (hide) e `__link-icon` (show inline) viraram dead code e foram removidas. Substituídas por 1 regra: `.lms-navbar__nav { display: none; }`. Desktop continua intacto (icon + texto). Bottom tab bar (#369) substituiu o destino dos links em mobile.
- **Pass 4 dark mode polish — botão do sino + badge das conquistas** (#370): 2 bugs residuais detectados pelo PO no smoke pós-v0.25.0. (1) Botão do sino (`.lms-bell__btn`) ficou branco em dark — passes 1-3 cobriram dropdown + items + title + unread, mas esqueci do trigger button em si (em `app.css` com `bg #fff` + `border #E5E7EB` + `color #374151` hardcoded). Fix: override em dark — bg=neutral-100, border=neutral-200, color=neutral-900; hover bg=neutral-200; `.lms-bell__badge` `border-color=card-bg` pra integrar com fundo escuro do navbar (era `#fff` hardcoded). (2) Badge "Realizou N cursos/tasks" branco/branco em conquistas unlocked — `.lms-achievement-card.is-unlocked .lms-achievement-card__badge` força `bg=rgba(255,255,255,0.95)` (sempre branco — overlay deliberado sobre o gradient colorido da família) + `color=var(--lms-neutral-900)`. Em dark, neutral-900 inverte → branco. Resultado: badge branco com texto branco. Fix: força `color=#111827` (preto fixo) só na variante `dark + unlocked` — bg do badge continua branco translúcido em ambos os temas (parte do design), texto também precisa ser fixo escuro.
- **Pill da patente mais arredondada** (#366, mergeada pós-v0.25.0): smoke do PO comparou visual e a pill da patente parecia quadrada perto da pill VOCÊ. `border-radius: 999px` já estava igual — problema era aspect ratio (padding 2px×10px com texto longo fazia a parte plana dominar). Fix: padding `2px 10px → 4px 14px` (modificador inline também: `1px 8px → 2px 10px`) + line-height fixo 18px → relativo 1.2.

### Mudanças internas / Tooling

- **`package.json`** bumpado para 0.26.0.

### Convenções consolidadas nesta janela

- **Bottom tab bar como nav primária mobile** — quando 5+ destinos primários precisam estar acessíveis em mobile e a top navbar não comporta, usar bar fixa na base é o padrão app-like esperado pelos usuários (Instagram, Twitter, etc.). Custo: `padding-bottom` no body + `env(safe-area-inset-bottom)` pro iPhone com notch + `z-index: 1030` calibrado abaixo do modal Bootstrap (z-1055). Restringir por papel quando o conjunto de destinos varia (aluno teve 4 tabs; teacher mobile é caso secundário pra revisitar quando aparecer demanda).
- **Path-based gate pra layout específico de papel** — quando o layout student-area é gatilhado por rota (não só por papel), centralizar a lista de paths no `$isStudentArea` do `layout.php` em vez de espalhar checks por cada page. Adicionar uma nova rota à área do aluno = 1 linha no array de paths + ajuste opcional no page handler pra skipar wrapper Bootstrap próprio. Padrão usado: `/profile`, `/notifications` — ambas existem pra teacher também mas só aluno recebe a versão student-area.
- **Cores hardcoded brancas em design unlocked precisam de texto também hardcoded** — quando um componente força bg branco translúcido por design (overlay sobre gradient colorido), o texto deve usar cor escura **fixa** (`#111827`) em vez de `var(--lms-neutral-900)`. Caso contrário, dark mode inverte o token e o texto vira branco invisível sobre branco. Regra: bg fixo ↔ color fixo.
- **Polish de dark mode é sempre N+1 passes** — 4 passes na E27 + ainda apareceram 2 fixes em pass 4 (sino btn + badge conquistas). Aceitar como custo natural — componentes que ninguém pisa no smoke do épico só viram exposição em uso real. Sempre rodar PR de fix isolado em vez de tentar agrupar com features novas.
...
#### Epic E27 — Dark mode (preferência aluno)

- **Schema users.theme + persistência + handler** (E27-01, #347): ALTER `users` ADD COLUMN `theme ENUM('light','dark') NOT NULL DEFAULT 'light'` AFTER `language` (idempotente via PREPARE/EXECUTE). Audit em `AuthController::authenticate` SELECT (replace_all cobriu as 2 ocorrências do query: login + reload). `$_SESSION['user']['theme']` populado com default `'light'`. Helper `current_user_theme(): string` em `src/helpers.php` com defesa pra non-aluno (teacher/super-admin/deslogado caem em `'light'`) + whitelist na leitura (defesa contra DB editado). Handler POST `/profile/theme` consistente com `/settings/language`: 403 quando `role !== 'student'` (defesa server-side; UI tampouco mostra toggle pra eles em #348), whitelist `'light'`/`'dark'` silenciosa (valores inválidos redirecionam sem flash), UPDATE DB + sincroniza session, flash success + redirect 303 pra referer same-host. 1 chave i18n PT/EN.
- **Aba "Aparência" no /profile** (E27-02, #348): nova aba (3ª) condicional ao `role='student'` — teacher/super-admin não vê. 2 radio cards (Claro / Escuro) com hint descritivo cada; pré-seleciona baseado em `current_user_theme()`. Submit POST `/profile/theme` com CSRF; flash success + reload aplica novo tema. Padrão visual reusado das opções de avatar em `/teacher/settings` (radio dentro de `label.card` com `border-primary` quando selecionado) — consistência sem CSS novo. 6 chaves i18n PT/EN (`profile.tab_theme` + 5 sub-chaves de `profile.theme.*`).
- **CSS dark variants em student-area.css** (E27-03, #349): `<body>` ganha class `lms-theme-dark` em `layout.php` quando `current_user_theme()==='dark'` E `isStudentArea` — sem flash of light theme (FOUT) porque é setado server-side antes do CSS carregar. ~210 linhas no fim de `student-area.css`: token overrides neutros (page-bg `#0F172A` slate-900, card-bg `#1E293B` slate-800, neutrais invertidos — texto agora claro, bordas escuras), shadows mais opacas (rgba 0.4-0.6) pra ficarem visíveis, **cores semânticas (success/warning/danger/violet/primary) preservadas em hue** — só os `-tint` viram `rgba(.., 0.18)` translúcidos. Overrides Bootstrap completos: `.card`, `.bg-white`, `.bg-light`, `.text-muted`, `.form-control`, `.form-select`, `.table` (via CSS vars), `.alert-*`, `.bg-*-subtle`, `.border-*-subtle`, `.list-group-item`. TinyMCE output (`.content-render`) coberto: text/links/pre/blockquote. Headings explicit color pra defesa contra inheritance perdida. Inputs com placeholder dim.
- **Polish + i18n + doc** (E27-04, #350): `comm` audit confirmou simetria perfeita pt/en — **1132 chaves em cada arquivo, zero divergência**. Doc dedicado novo `doc/21-dark-mode.md` com TL;DR + diagrama de fluxo (Auth → session → layout → CSS) + CSS strategy (token overrides) + decisões consolidadas + tabela de defesa em 5 camadas + edge cases tratados.

### Correções

- **Polish dark mode pass 1** (#356): 5 componentes LMS-específicos não cobertos no E27-03 ficaram light em dark — sidebar sub-cards (`.lms-xp-block`, `.lms-ranking-block`, `.lms-achievements-block` com `background: #FAFAFA` hardcoded → trocado por `var(--lms-neutral-100)`); navbar (`.lms-navbar` com `background: #fff` hardcoded em app.css → override completo: bg, brand, links, dropdown, divider, lms-navbar__logo--branded); tabs Bootstrap nav-tabs (`.nav-link.active` com bg branco hardcoded → override + inativo color neutral-500); hero ring (`.lms-progress-ring::before` com `background: #fff` hardcoded em app.css → override pra var(--lms-card-bg) em outros rings; hero ring específico mantido branco translúcido sobre gradient colorido com label `#111827` fixo); modal/sino (`.modal-content`, `.lms-bell__dropdown`, `.lms-bell__item` + `.btn-close` com `filter: invert(1)` pro X aparecer em dark).
- **Polish dark mode pass 2** (#357): 4 fixes residuais — sidebar sub-cards perderam contorno (neutral-100 ficou muito próximo de card-bg → adicionada `border: 1px solid var(--lms-neutral-200)` explícita); botão idioma na navbar continuou branco (pass 1 cobriu só `color`; bg #fff e border #E5E7EB hardcoded continuavam → override bg + border); hero ring continuou ilegível (pass 1 errou seletor — usou espaço entre classes, mas `lms-progress-ring` e `lms-course-hero__ring` coexistem no MESMO elemento → corrigido pra `.lms-progress-ring.lms-course-hero__ring::before` sem espaço); modal de notificações com títulos pretos (`.lms-bell__item-title`, `-text`, `-time`, `--unread` com cores hardcoded em app.css → override completo + `.modal-title` defensivo + unread com violet-tint translúcido).
- **Polish dark mode pass 3** (#358): 2 fixes finais — título "Notificações" no dropdown do sino continuou preto (`.lms-bell__dropdown-title` com `color: #111827` hardcoded — pass 1/2 cobriram só `.lms-bell__item-title`, não o título do header geral → override `.lms-bell__dropdown-title` + `.lms-bell__empty` + borders do header/footer); "100%" estourando o anel do hero do curso (`font-size: 20px` com 4 chars não cabia em 112px independente de tema — só foi notado em dark depois do label ficar legível → reduzido pra 17px, afeta light + dark).
- **`template_reports/` excluído do upload pra prod** (#345, mergeado pós-v0.23.0): pasta gitignored mas `ftp-deploy.mjs` usa allowlist própria e enviou os 2 arquivos do PO sem necessidade. Adicionado `template_reports` ao `EXCLUDE_PATHS`.

### Mudanças internas / Tooling

- **`package.json`** bumpado para 0.24.0.
- **Doc roadmap**: `doc/15-roadmap-pos-mvp.md` ganhou F21/E30 (refazer template Skills Hub) — pendência pós-v0.23.0 que entra no final da fila.

### Convenções consolidadas nesta janela

- **CSS dark mode via token overrides em block único** — quando o sistema de design usa CSS custom properties bem-definidos (`--lms-card-bg`, `--lms-neutral-*`), implementar dark mode é 90% sobrescrever os tokens num seletor `body.theme-dark { ... }`. Componentes que consomem tokens herdam automático. Restantes 10% são overrides Bootstrap (`.bg-white`, `.text-muted`, etc.) que não consomem nossos tokens — precisam regra explícita.
- **Cores semânticas preservadas em hue, tints translúcidos** — pra manter identidade visual entre temas, success/warning/danger/primary devem ter mesmo hue em ambos. Só os `-tint` (fundos sutis) trocam: claro usa cor pastel hardcoded; dark usa `rgba(..., 0.18)` translúcido sobre o fundo escuro. Resultado: badges/alertas reconhecíveis em qualquer tema.
- **Smoke iterativo de UI** — mudanças visuais grandes (dark mode) precisam 2-3 passes de polish após o smoke do PO, porque é difícil prever quais componentes têm cores hardcoded sem rodar visualmente. Aceitar isso como custo natural do processo: implementar 1 PR base + 2-3 fixes pequenos sequenciais é mais rápido que tentar prever tudo upfront.
- **Seletor CSS com classes coexistentes vs descendentes** — bug recorrente: quando 2+ classes coexistem no mesmo elemento (`<div class="A B">`), combinar sem espaço (`.A.B`); pra descendente, com espaço (`.A .B`). Falhar nesse detalhe faz a regra mais específica perder pra uma mais genérica silenciosamente. Sempre validar markup HTML quando regra parece "não estar aplicando".
- **Defesa em camadas pra preferências de usuário** — quando uma preferência (ex.: tema) é restrita a um papel: gate na UI (mostrar toggle só pra esse papel), defesa server-side no handler (403 pros outros), helper que retorna default pra non-aplicáveis (independente do DB), restrição na aplicação visual (class no body só quando combinação de role + preferência bate). 4 camadas pra blindar contra bypass acidental ou DB editado.

## [0.23.0] — 2026-04-26

Vigésima terceira release. Escopo: **Epic E26 inteiro — Reports PDF (Actvet+LO only)** (5 stories, F17) + 1 fix do FTP deploy script (throttle + reconexão automática) viabilizando deploys grandes na Hostinger compartilhada. Fecha o ciclo Actvet-específico do roadmap pós-MVP estendido (F14–F17 todos entregues). Pendência conhecida da v1 do template Skills Hub anotada — refazer planejado pra E30 (F21).

### Novas funcionalidades

#### Epic E26 — Reports PDF (Actvet+LO only)

- **Schema + composer dompdf + assets do template Skill Hub** (E26-01, #331): 2 ALTERs idempotentes (`courses.report_mode ENUM('disabled','skill_hub') DEFAULT 'disabled'` AFTER `grading_mode`; `evaluation_submissions.report_pdf_path VARCHAR(500) NULL` AFTER `quiz_snapshot`) via PREPARE/EXECUTE. **`composer require dompdf/dompdf v3.1.5`** (+ 5 transitivas: masterminds/html5, php-css-parser, thecodingmachine/safe, php-svg-lib, php-font-lib — todas PHP-puro, compatível com Hostinger compartilhado). Template entregue pelo PO em `template_reports/CU_SKILL_HUB.htm` migrado pra `public/assets/report-templates/skill_hub/template.html` + `images/` (paths `<img src>` reescritos via `sed`). `storage/reports/` criado com `.gitkeep` + `.htaccess deny` (mesmo padrão de `storage/logs`/`storage/cache`). `.gitignore` ganhou 2 regras: `storage/reports/*` (versiona só .gitkeep+.htaccess) + `template_reports/` (working dir do PO).
- **`ReportService::generate(submissionId)`** (E26-02, #332): API com 5 status semânticos (`ok`/`not_found`/`not_eligible`/`not_ready`/`render_failed`). Validação Actvet+LO+report_mode+5 LOs com grade. **`loadContext`** em 1 query JOIN + 2 satélites existentes (`LearningOutcome::findByCu` + `findGradesBySubmission`) — sem N+1. **`strtr`** ao invés de `str_replace` (1 pass, sem cascata acidental). Helpers `formatScore` (×10 com regra exata da spec — `8.0`→`'80'`, `8.5`→`'85'`, `8.52`→`'85,2'`) e `formatWorkload` (`30h`). **`htmlspecialchars`** defensivo nas descrições/nomes do banco. **dompdf gated**: `chroot=templateDir` + `isRemoteEnabled=false` + `setBasePath` no template dir → sem path traversal nem SSRF. Variáveis fora do catálogo ficam literais no PDF (debug-friendly, decisão da spec). Path retornado relativo a LMS_ROOT; re-geração sobrescreve (naming determinístico `eval_X_student_Y_attempt_N.pdf`).
- **Select `report_mode` no curso (Actvet+LO only)** (E26-03, #333): partial Alpine (`x-data` + `x-model` + `x-show`) — trocar `grading_mode` esconde report_mode em tempo real. **Defesa server-side em 2 condições**: `TeacherCoursesController::validate` força `'disabled'` silenciosamente quando `!isActvet` OU `gradingMode !== 'learning_outcomes'`. Audit chain consistente com E25-01: `Course::findForTenant`/`create`/`update` + `StudentCurriculum::buildForCourse` ganham `report_mode`. 4 chaves i18n PT/EN.
- **Trigger pós-último-LO + endpoint download** (E26-04, #334): hook em `EvaluationSubmissionService::gradeByLo` chama `ReportService::generate` **fora da transação** — best-effort: falha do dompdf NÃO afeta o feedback já commitado (vai pra `error_log`). UPDATE `report_pdf_path` com retorno do service. Rota nova `/teacher/evaluation/{id}/submission/{student_id}/report.pdf` em `src/routes.php`. Page handler `report-pdf.php` valida ownership via `findForGrading` (JOIN tenant) + **path traversal defense** com `realpath` confinado a `LMS_ROOT/storage/reports`. Headers HTTP completos (`Content-Type: application/pdf` + `Content-Length` + `Content-Disposition: inline` + `X-Content-Type-Options: nosniff` + `Cache-Control: private no-cache`). `target=_blank rel=noopener` no link (anti reverse tabnabbing). Aluno **não vê** o link em lugar nenhum (`/student/evaluation/show.php` nem busca `report_pdf_path`). Audit em `EvaluationSubmission::findForGrading` (`report_pdf_path` no SELECT). Botão "Baixar Report" texto puro (sem ícone — bootstrap-icons CSS não carrega fora student area; pendência [E26-04] documentada). 1 chave i18n PT/EN.
- **Polish + i18n + doc** (E26-05, #335): `comm` audit confirmou 1125 chaves em ambos pt/en, zero divergência. **3 chaves órfãs** do `ReportService` (`reports.err.{template_missing,outdir_missing,render}`) detectadas e adicionadas pra completude (hoje só vão pra log; futuros callers podem exibir). Doc dedicado `doc/19-reports.md` com TL;DR + diagrama de fluxo + catálogo de variáveis + tabela de defesa em 5 camadas + edge cases tratados.

### Correções

- **FTP deploy throttle + reconexão automática contra rate-limit Hostinger** (#338): plano compartilhado FTPS derruba conexão após N uploads consecutivos (FIN packet ou ECONNRESET). Aconteceu reproduzivelmente em deploys grandes (ex.: vendor/ inicial do dompdf com ~600 arquivos — primeira tentativa falhou após 4, segunda após 68). Fix: `THROTTLE_MS=80` entre uploads + `RECONNECT_EVERY=50` (reconexão proativa antes do servidor fechar) + detecção de erro de conexão + 1 retry por arquivo (FIN, ECONNRESET, EPIPE, ETIMEDOUT, "Client is closed"). Sem mudança de comportamento pra deploys pequenos (<50 arquivos): 80ms imperceptível por arquivo. Resolveu — vendor/ inicial subiu 571 arquivos sem queda no terceiro deploy.

### Mudanças internas / Tooling

- **`package.json`** bumpado para 0.23.0.
- **`composer.json` + `composer.lock`** ganharam `dompdf/dompdf ^3.1` (+ 5 transitivas). Em prod, `vendor/` (16 MB) precisa ser feito local + subir via FTP (Hostinger compartilhado não tem composer no SSH); deploy script já trata `vendor/` como pasta normal de upload.
- **`.gitignore`**: regras novas pra `storage/reports/*` (versiona só placeholders) + `template_reports/` (working dir do PO). Também `.claude/scheduled_tasks.lock` (runtime do harness).
- **`chore(reports)`**: ícone `bi-file-earmark-pdf` removido do botão "Baixar Report" — bootstrap-icons CSS não carrega fora student area (bug existente também afeta `bi-trophy`/`bi-award` do header navbar mobile pra prof/admin; resolver junto com E29).
- **Doc roadmap atualizado em PR separado pré-release**: F21 → E30 adicionado pro final do roadmap (refazer template Skills Hub em HTML/CSS limpo, fiel ao PDF de referência do PO; backfill de submissions já corrigidas via loop de `generate`).

### Pendências de produção (anotadas em doc/99 pré-release)

- **`[E26 v1]` Template Skills Hub renderiza diferente do Word original** (impacto alto pra UX): a v1 do template foi gerada via Word "Save as Web Page Filtered" e usa marcação MSO-específica que dompdf não cobre fielmente. Pipeline (trigger, storage, endpoint, segurança) está OK e funciona com qualquer template. **Ação:** E30 (F21) refaz template em HTML/CSS limpo + backfill — sem nova migração de schema necessária.

### Convenções consolidadas nesta janela

- **Composer dependency PHP-puro em hospedagem compartilhada** — quando trazer lib que precisa de runtime (ex.: `dompdf/dompdf`), preferir libs PHP-puro (sem deps externas tipo libreoffice/wkhtmltopdf/imagemagick). Hostinger compartilhado não tem SSH na maioria dos planos — `composer install` precisa rodar local + subir `vendor/` via FTP. Documentar isso no PR pra evitar fricção pós-merge.
- **Resiliência de deploy contra rate-limit** — quando o destino é hospedagem compartilhada, deploys grandes (centenas de arquivos) frequentemente caem no meio. Fix de baixa fricção: throttle de poucos ms entre uploads + reconexão proativa periódica + retry simples no erro. Evita necessidade de subir manualmente via cliente FTP.
- **Trigger fora da transação como best-effort** — quando uma feature opcional é encadeada num fluxo crítico (ex.: gerar PDF após salvar feedback), rodar fora da `Database::tx` mantém o fluxo principal robusto: falha da feature opcional vai pra `error_log` mas não derruba o que o usuário já commitou. Service da feature deve gatear elegibilidade sozinho (retornar status semânticos como `not_eligible`) pra caller não precisar duplicar a lógica.
- **Naming determinístico de arquivos pra idempotência** — quando o mesmo evento pode disparar geração múltiplas vezes (re-correção, retry), nomear por chaves estáveis (ex.: `eval_X_student_Y_attempt_N.pdf`) faz a re-geração sobrescrever sem precisar deletar/track de versões anteriores.
- **`realpath` + `str_starts_with($basePath)` pra defesa de path traversal** — quando o servidor lê um path do banco e serve via `readfile`, sempre conferir que o path resolvido cai dentro do diretório esperado. Defesa contra DB editado manualmente ou bug de geração de path.
- **Documentar trade-offs de migração no início, não no smoke** — ao recomendar caminho técnico (ex.: "Save as HTML Filtered" pra migrar template Word), explicitar que é caminho de menor esforço **vs** fidelidade visual. PO precisa saber se vai ter que refazer; descobrir só no smoke gera retrabalho e perda de confiança no plano.

## [0.22.0] — 2026-04-26

Vigésima segunda release. Escopo: **Epic E25 inteiro — Learning Outcomes (Actvet-only)** (6 stories, F16). Modo de avaliação alternativo a "nota única": notas por critério (5 LOs por UC) com média = nota final. Habilita o modelo "skills competency framework" requisitado pelo PO Actvet e estabelece a base pra E26 (Reports PDF).

### Novas funcionalidades

#### Epic E25 — Learning Outcomes (Actvet-only)

- **Schema + LearningOutcome model + grading_mode em courses** (E25-01, #317): coluna nova `courses.grading_mode ENUM('grade','learning_outcomes') DEFAULT 'grade'` (ALTER idempotente via PREPARE/EXECUTE); 2 tabelas novas — `learning_outcomes(id, cu_id FK CASCADE, description VARCHAR(500), position INT)` e `evaluation_submission_lo_grades(submission_id, lo_id FK CASCADE em ambas direções, grade DECIMAL(3,1) CHECK 0..10, PK composite)`. Model `LearningOutcome` com `findByCu`, `replaceForCu` (transação atômica DELETE + N×INSERT com position 1..N), `countByCu`, `findGradesBySubmission`. Audit em `Course::findForTenant/create/update` + `StudentCurriculum::buildForCourse`. Form `/teacher/courses/{id}/edit` ganha select "Modo de avaliação" condicional ao tenant Actvet; default `'learning_outcomes'` em curso novo Actvet. **Defesa server-side**: `TeacherCoursesController::validate` aceita `is_actvet` + força `'grade'` se não-Actvet (ignora silenciosamente POST manipulado). 4 chaves i18n PT/EN.
- **Cadastro de 5 LOs por UC** (E25-02, #318): página nova `/teacher/cu/{id}/learning-outcomes` com 5 textareas obrigatórios (max 500 chars cada). **Gate em 3 camadas**: tenant ownership via `CompetenceUnit::findForTenant` (audit em `course_grading_mode`) + `tenant.is_actvet=1` + `course.grading_mode='learning_outcomes'` — qualquer falha vira 403 silencioso. Save atômico via `LearningOutcome::replaceForCu`. Pré-popula com LOs existentes; preserva valor digitado em erro. Curso arquivado vira read-only. Botão "Editar critérios" no `show.php` da CU com badge `X/5` mostrando progresso. Breadcrumb completa. 10 chaves i18n PT/EN.
- **Form de feedback adapta — 5 inputs por LO + média auto** (E25-03, #319): tela `/teacher/evaluation/{id}/submission/{sid}` detecta `course.grading_mode` (audit em `EvaluationSubmission::findForGrading`). Em LO mode com 5 LOs cadastrados, renderiza UI alternativa com 5 inputs number 0-10 step 0.1; **Alpine `x-data` calcula média em tempo real** (respeita `,` e `.` como separadores). UC sem 5 LOs (curso virou LO depois): alerta com link pro cadastro + bloqueia POST. **Refactor `applyGrade(PDO, ...)` privado** centraliza a lógica core (clamp retry ≥6, UPDATE submission grade, XP award ≥8 idempotente) — `grade()` clássico e `gradeByLo()` chamam o mesmo helper. `gradeByLo` usa `REPLACE INTO` (PK composite torna re-correção idempotente sem DELETE). Pré-popula notas existentes via `findGradesBySubmission`. UI clássica intacta em curso `grade` mode (zero regressão). 9 chaves i18n PT/EN.
- **Esconder Quiz no select de tipo de evaluation em LO mode** (E25-04, #320): `_form.php` (compartilhado entre new e edit) esconde a opção `quiz` quando `$isLoMode && $mode === 'new'`. Em `mode='edit'` preserva visibilidade pra evaluations pré-existentes tipo quiz quando o curso virou LO depois (type é imutável em edit). Defesa server-side em `new.php` rejeita POST com `type=quiz` em LO mode. Hint adaptativo `type_hint_lo` explica "só Projeto é compatível". Atividades-quiz seguem disponíveis (escopo é só evaluations). Audit em `Evaluation::findForTenant` (`course_grading_mode`). 2 chaves i18n PT/EN.
- **Aluno vê critérios avaliados + nota por LO pós-feedback** (E25-05, #321): `/student/evaluation/{id}` em curso LO mode exibe bloco "Critérios avaliados" logo após o brief PDF. **Antes do feedback**: lista os 5 LOs read-only com hint orientativo. **Após feedback**: cada LO ganha badge colorido com a nota (verde ≥6, vermelho <6) — visual imediato de pontos fortes/fracos. Curso `grade` mode: bloco não aparece (zero regressão). UC sem 5 LOs: defesa silenciosa (não renderiza). Audit em `EvaluationSubmission::findForStudentEvaluation`. 3 chaves i18n PT/EN.
- **Polish + i18n + doc** (E25-06, #322): doc dedicado novo `doc/18-learning-outcomes.md` com TL;DR + schema + tabela de defesa em camadas + padrão `applyGrade` + audit chain de `course.grading_mode` (6 models) + edge cases tratados. **i18n audit objetivo via `comm`**: 1117 chaves em ambos pt/en, zero divergência; grep no `src/` validou que cada chave nova do épico está em uso (zero órfã). Pendências anotadas em `doc/99` (cascade replace + UC sem 5 LOs). Hardening defensivo do hook em 2 commits — removidas menções a credenciais sensíveis (SMTP/Judge0/FTP) em docs históricas.

### Mudanças internas / Tooling

- **`package.json`** bumpado para 0.22.0.
- **Schema impact aplicado em prod** durante o épico: 1 coluna em `courses` + 2 tabelas novas. Sem nova mudança neste release fechamento.
- **Doc roadmap** atualizado em PR separado pré-release: F18-F20 adicionados (E27 dark mode, E28 patentes ranking, E29 identidade visual unificada) — fila pós-Actvet.

### Convenções consolidadas nesta janela

- **Defesa server-side em camadas** — toda regra de visibilidade no UI também é validada no backend, pra resistir a POST manipulado. Documentar a tabela de camadas no doc dedicado do épico ajuda na manutenção (ex.: tabela em `doc/18-learning-outcomes.md`).
- **Helper privado pra lógica core compartilhada** — quando 2 fluxos convergem na mesma persistência (ex.: `grade()` e `gradeByLo()` ambos UPDATE submission + XP), extrair `private static function helper(PDO $pdo, ...)` que roda dentro de uma `Database::tx` aberta pelo caller. Evita aninhar transações; mantém uma única fonte de verdade pra regras compartilhadas.
- **`REPLACE INTO` com PK composite pra idempotência** — quando o caller pode re-executar (ex.: re-correção do feedback), `REPLACE INTO` aproveita a PK composite (`submission_id+lo_id`) pra sobrescrever sem precisar `DELETE` explícito. Ganha simplicidade e atomicidade.
- **Audit chain pra evitar query extra em cada page** — quando uma coluna nova (ex.: `course.grading_mode`) precisa ser consumida por múltiplas pages, adicioná-la nos SELECTs explícitos relevantes (com alias defensivo tipo `course_grading_mode` quando o nome puro pode colidir) é mais limpo que carregar o curso em cada caller. Documentar a chain (qual model expõe pra qual page) no doc do épico.
- **Hide condicional só em `new` quando o campo é imutável em `edit`** — se um campo (ex.: `type` da evaluation) é setado na criação e nunca muda depois, esconder a opção indesejada só faz sentido em `mode='new'`. Em `edit` preserva visibilidade pra dados pré-existentes (ex.: evaluation criada como quiz antes do curso virar LO).
- **i18n audit via `comm`** — `comm -23 pt-keys en-keys` lista chaves só em PT (faltando em EN); `comm -13` lista o inverso. Combinado com grep no `src/` por chaves órfãs (existem no lang mas ninguém usa), fecha o ciclo de qualidade i18n no fim do épico.
